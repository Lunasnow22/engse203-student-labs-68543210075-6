# API Contract — นิรันดร์รักษ์ อนุสนธิ์
...
| **Base URL (ใช้งานจริง)** | ยังไม่มี (อยู่ระหว่างการพัฒนา) |
| **รูปแบบข้อมูล** | JSON (`Content-Type: application/json`) |
| **การยืนยันตัวตน** | ยังไม่มีในเวอร์ชันนี้ |
| **ผู้จัดทำ** | นิรันดร์รักษ์ อนุสนธิ์ 68543210075-6 |
| **วันที่ปรับปรุงล่าสุด** | 19/09/2569 |
...
```json
{
  "error": "สถานะต้องเป็น pending, in-progress หรือ completed เท่านั้น"
}
```
...
**ทำอะไร** ลบคำร้องออกจากระบบตามรหัส `id` ที่ระบุ
...
|---|---|---|:---:|
| 1.0.0 | 14/09/2569 | เวอร์ชันแรก — GET, POST, DELETE | — |
| 2.0.0 | 19/09/2569 | เพิ่ม `PUT /api/requests/:id` และ CORS | ไม่ |
